...
icp_localization::icp_localization()
{
  ...
  wheel_odom_old.pose.pose.orientation.z = -2.186351;
  ros::spinOnce();
  // printf("init done \n");
}
...
void icp_localization::cb_matching(const sensor_msgs::PointCloud2 &msg)
{
  ...
  pcl::PointCloud<pcl::PointXYZI>::Ptr filtered_cloud4(new pcl::PointCloud<pcl::PointXYZI>);
  ...
  map_voxel.setLeafSize (0.2f, 0.2f, 0.2f);
