...
icp_localization::icp_localization()
{
  ...
  nh.getParam("/NDT_localization_2/result_save_path", result_save_path);
  ...
  {
    PCL_ERROR ("Couldn't read file nuscenes_map_downsample.pcd \n");
    exit(0);
  }
  std::cout << "Loaded "
...
void icp_localization::cb_matching(const sensor_msgs::PointCloud2 &msg)
{
  ...
  sor0.setLeafSize (0.25f, 0.25f, 0.25f);
  ...
  double dist = 0.75;
  // if (num >= 8) dist = 0.9;
  // else if (num >= 2) dist = 1.5;
  // else if (num >= 5) dist = 1.1;
  icp.setMaximumIterations (1000);              //最大迭代次數
  icp.setTransformationEpsilon (1e-12);         //上次轉換與當前轉換的差值(early stop)
  icp.setMaxCorrespondenceDistance (dist);         //Distance to see
