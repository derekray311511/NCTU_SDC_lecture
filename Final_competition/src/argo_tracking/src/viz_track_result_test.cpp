...
// Variable to save previous pose and current pose
double pre_pose_x, pre_pose_y, pre_pose_z, pre_yaw;
double cur_pose_x, cur_pose_y, cur_pose_z, cur_yaw;
double displacement, yaw_change;
...
void Object::update(float displacement, float yaw_change) {
...
tf::Transform broadcast_worldToBasicLink(Json::Value pose)
{
	// Save pose
	tf::Quaternion tfqt;
	cur_pose_x = pose["translation"][0].asDouble();
	cur_pose_y = pose["translation"][1].asDouble();
	cur_pose_z = pose["translation"][2].asDouble();
	...
	if (frame_count == 0) {
		...
		pre_pose_z = cur_pose_z;
	}
	else {
		displacement = sqrt(pow(cur_pose_x - pre_pose_x, 2) + pow(cur_pose_y - pre_pose_y, 2));
		yaw_change = -(cur_yaw - pre_yaw);
		pre_yaw = cur_yaw;
		pre_pose_x = cur_pose_x;
		pre_pose_y = cur_pose_y;
		pre_pose_z = cur_pose_z;
	...
	double pose_x = pose["rotation"][0].asDouble();
	double pose_y = pose["rotation"][1].asDouble();
	double pose_z = pose["rotation"][2].asDouble();
	double pose_w = pose["rotation"][3].asDouble();
	// printf("pose_x: %lf\n", pose_x);
	// printf("pose_y: %lf\n", pose_y);
	// printf("pose_z: %lf\n", pose_z);
	// printf("pose_w: %lf\n", pose_w);
...
int update_points(int id, int hist, double displacement, double yaw_change) 
{
	double x = trajectory_points[id][hist][0] * cos(yaw_change) + trajectory_points[id][hist][1] * sin(yaw_change) - displacement;
	double y = -trajectory_points[id][hist][0] * sin(yaw_change) + trajectory_points[id][hist][1] * cos(yaw_change);
	trajectory_points[id][hist][0] = x;
	trajectory_points[id][hist][1] = y;
	trajectory_points[id][hist][2] = 0;
	return 1;
}
...
int add_point(int id, double point[3])
{
	...
	point_temp = point_temp;	// Save points on scan frame
	...
	if (id == temp) {
		...
		for (int i = 10; i < 20; i ++) {
			printf("%d: %lf, %lf, %lf\n", i, trajectory_points[temp][i][0], trajectory_points[temp][i][1], trajectory_points[temp][i][2]);
		}
		// printf("Trans point: %lf %lf %lf\n", point[0], point[1], point[2]);
		printf("Displacement = %lf\n", displacement);
		printf("Yaw_change = %lf\n", yaw_change);
	}
	return hist_len + 1;
}
...
void initialize_marker(Json::Value label_array, Json::Value det_array){
	...
	for (int i=0; i<label_array.size(); i++){
		...
		marker_hist.header.frame_id = "scan";
			...
			for(int m=0; m<id_record.size(); m++){
				if ( !( (track_label_uuid.second).compare(id_record.at(m))) ) {
					...
					for (int j = 0; j < hist_len; j++) {
						...
						marker_hist.color.b = id / (float)id_record.size();
