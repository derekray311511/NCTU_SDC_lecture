...
FRAME_ID = "scan" # the base coordinate name in rviz
RATE = 10
...
def publish_location_(loc_pub, locations):
    marker_array = MarkerArray()
    ...
    for p in locations:
        ...
        marker.color.r = 1.0
        if i % 2 == 1:
            marker.color.g = 0.0 + i / len(locations)
        else:
            marker.color.b = 0.0 + i / len(locations)
        i = i + 1
    ...
    #if log:
def publish_location(loc_pub, tracker, centers):
    marker_array = MarkerArray()
    count = 0
    #print(type(centers))
    for track_id in centers:
        #print(centers[track_id][0]/ 150.0)
        count = count+1
        marker = Marker()
        marker.header.frame_id = FRAME_ID
        marker.header.stamp = rospy.Time.now()
        ...
        marker.color.r = 0.7
        marker.color.g = abs(centers[track_id][0]) / 100.0
        marker.color.b = abs(centers[track_id][1]) / 15.0
        marker.color.a = 1.0
        marker.scale.x = 0.2
