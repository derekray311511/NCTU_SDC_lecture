...
icp_localization::icp_localization()
{
  ...
  std::cerr << "Before StatisticalOutlierRemoval filtered: " << map->points.size() << endl;
  // Filtering
  pcl::StatisticalOutlierRemoval<pcl::PointXYZI> filter_1;
  filter_1.setInputCloud(map);
  filter_1.setMeanK (50);
  filter_1.setStddevMulThresh (1.0);
  filter_1.filter(*map);
  std::cerr << "After  StatisticalOutlierRemoval filtered: " << map->points.size() << endl;
  ...
  pub_matched_lidar_scan = nh.advertise<sensor_msgs::PointCloud2>("pub_matched_lidar_scan", 20);
...
void icp_localization::cb_matching(const sensor_msgs::PointCloud2 &msg)
{
  ...
  pass.setFilterLimits(1700.0, 2000.0);
  pass.filter(*crop_map1);//儲存處理之後的點雲

  pass.setInputCloud(crop_map1);
  pass.setFilterFieldName("y");
  pass.setFilterLimits(700.0, 1000.0);
  pass.filter(*crop_map2);//儲存處理之後的點雲

  // // StatisticalOutlierRemoval filter
  // pcl::PointCloud<pcl::PointXYZI>::Ptr filtered_crop_map(new pcl::PointCloud<pcl::PointXYZI>);
  // std::cerr << "Before StatisticalOutlierRemoval filtered: " << crop_map2->points.size() << endl;
  // // Filtering
  // pcl::StatisticalOutlierRemoval<pcl::PointXYZI> filter_1;
  // filter_1.setInputCloud(crop_map2);
  // filter_1.setMeanK (50);
  // filter_1.setStddevMulThresh (1.0);
  // filter_1.filter(*filtered_crop_map);
  // std::cerr << "After  StatisticalOutlierRemoval filtered: " << filtered_crop_map->points.size() << endl;
  ...
  filter_1.setMeanK (50);
  filter_1.setStddevMulThresh (1.0);
  filter_1.filter(*filtered_cloud);
  std::cerr << "After  StatisticalOutlierRemoval filtered: " << filtered_cloud->points.size() << endl;
  ...
  // icp.setInputTarget(filtered_crop_map);
  // set ICP parameters ================================================
  icp.setMaximumIterations (1000);              //最大迭代次數
  icp.setTransformationEpsilon (1e-12);         //上次轉換與當前轉換的差值(early stop)
  icp.setMaxCorrespondenceDistance (2);         //Distance to see
  icp.setEuclideanFitnessEpsilon (0.001);        //前後兩次迭代誤差的差值(early stop)
