...
class icp_localization
{
  private:
    ...
    Eigen::Matrix4f trans_world_car;
...
icp_localization::icp_localization()
{
  ...
  nh.getParam("/NDT_localization_3/result_save_path", result_save_path);
  ...
  diff_point.setY(0.0);
  diff_point.setZ(0.0);
  wheel_odom_old.pose.pose.orientation.x = 0.0;
  wheel_odom_old.pose.pose.orientation.y = 0.0;
  wheel_odom_old.pose.pose.orientation.z = 0.0;
  wheel_odom_old.pose.pose.orientation.w = 1.0;
...
void icp_localization::get_diff(const nav_msgs::Odometry &wheel_odom_current)
{
  Eigen::Matrix4f mat4;
  tf::Matrix3x3 mat3x3;
  tf::Quaternion tfq2_0;
  // mat4 = get_transfrom("world");
  mat4 = trans_world_car;
  tf::StampedTransform trans0;
  mat3x3.setValue(mat4(0,0), mat4(0,1), mat4(0,2),
                  mat4(1,0), mat4(1,1), mat4(1,2),
                  mat4(2,0), mat4(2,1), mat4(2,2));
  mat3x3.getRotation(tfq2_0);
  trans0.setRotation(tfq2_0);

  tf::Vector3 diff_point_temp;
  tf::Vector3 zero_vec;
  zero_vec.setValue(0, 0, 0);
  ...
  diff_point_temp.setX(diff_x);
  diff_point_temp.setY(diff_y);
  diff_point_temp.setZ(diff_z);
  tf::Quaternion tfq2;
  ROS_INFO("Diff_point_temp: %lf", diff_point_temp.getX());
  tfq2.setX(wheel_odom_old.pose.pose.orientation.x);
  tfq2.setY(wheel_odom_old.pose.pose.orientation.y);
  tfq2.setZ(wheel_odom_old.pose.pose.orientation.z);
  tfq2.setW(wheel_odom_old.pose.pose.orientation.w);

  tf::StampedTransform trans;
  trans.setRotation(tfq2);
  trans0.setOrigin(zero_vec);
  diff_point = trans0.inverse() * trans.inverse() * diff_point_temp;
  ROS_INFO("Trans1: %lf, %lf, %lf, %lf", tfq2.getX(), tfq2.getY(), tfq2.getZ(), tfq2.getW());
  tfq2 = trans0.getRotation();
  ROS_INFO("Trans0: %lf, %lf, %lf, %lf", tfq2.getX(), tfq2.getY(), tfq2.getZ(), tfq2.getW());
  tf::Vector3 origin_temp;
  origin_temp = trans0.getOrigin();
  ROS_INFO("Trans0: %lf, %lf, %lf", origin_temp.getX(), origin_temp.getY(), origin_temp.getZ());
  ROS_INFO("Diff_point: %lf", diff_point.getX());
  ...
  wheel_odom_old.pose.pose.orientation.w = wheel_odom_current.pose.pose.orientation.w;
}
...
void icp_localization::cb_matching(const sensor_msgs::PointCloud2 &msg)
{
  ...
  // pass.setFilterLimits(1.5, 8);
  // pass.filter(*crop_map2);//儲存處理之後的點雲
  pass.setInputCloud(crop_map2);
  pass.setFilterFieldName("z");
  // pass.setFilterLimits(initial_guess(2, 3) - 2.5, initial_guess(2, 3) + 20.5);
  pass.setFilterLimits(-2, 16.5);
  pass.filter(*crop_map2);//儲存處理之後的點雲

  pcl::VoxelGrid<pcl::PointXYZI> map_voxel;
  map_voxel.setInputCloud (crop_map2);
  // map_voxel.setFilterFieldName("z");
  // map_voxel.setFilterLimits(0.5, 2.5);
  // map_voxel.setFilterLimitsNegative(true);
  ...
  ROS_INFO("Lidar Scan PointCloud transformed to car. Matching......");
  ...
  trans_world_car = get_transfrom("world");
  ...
  if (num >= 3) {
    ...
    diff_yaw = (curr_yaw - last_yaw) * 0.75;
    ...
    ROS_INFO("Diff_point: %lf", diff_point.getX());
    initial_guess(0,3) = initial_guess(0,3) + diff_point.getX();
    initial_guess(1,3) = initial_guess(1,3) + diff_point.getY();
    initial_guess(2,3) = initial_guess(2,3) + diff_point.getZ();
  ...
  ROS_INFO("DIFF_X: %lf", diff_point.getX());
  ROS_INFO("DIFF_Y: %lf", diff_point.getY());
  ROS_INFO("DIFF_Z: %lf", diff_point.getZ());
